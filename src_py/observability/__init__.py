    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                logger.info(f"{metric_name} completed", extra={"module": metric_name, "duration_ms": round(duration, 2), "status_code": 200})
                return result
            except Exception as e:
                duration = (time.perf_counter() - start) * 1000
                logger.error(f"{metric_name} failed: {e}", extra={"module": metric_name, "duration_ms": round(duration, 2), "status_code": 500}, exc_info=True)
                raise
        return wrapper
    return decorator
